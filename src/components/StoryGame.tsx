		...
		<div className="relative w-full h-screen overflow-hidden">
			...
			<div className="absolute top-4 right-4 z-50 flex items-center space-x-4">
				...
				{session ? (
					<div className="flex items-center space-x-3">
						<span className="text-amber-200 antique-text">Welcome, {session.user?.name}</span>
						<button
					...
					<div className="flex items-center space-x-2">
                       ...
                       transition-all duration-300 antique-text"
						...
						<Link
							href="/auth/signup"
							className="px-4 py-2 bg-amber-300/70 backdrop-blur-sm border border-amber-400/50
                       rounded-lg text-amber-900 hover:text-amber-100 hover:border-amber-500/60
			...
			{error && (
				<div className="absolute top-4 right-4 z-50">
					<div
						className="bg-red-900/80 backdrop-blur-sm border border-red-600/50
			...
			{showHistory && history.length > 0 && (
				...
				>
					<div
						className="bg-gradient-to-br from-amber-900/90 to-amber-800/70
                        backdrop-blur-md rounded-xl border border-amber-600/50
                        p-6 max-w-4xl max-h-[80vh] overflow-y-auto m-4"
					>
						<div className="flex justify-between items-center mb-6">
							<h2 className="text-2xl font-bold gold-text decorative-text">Story History</h2>
							<button
						...
						<div className="space-y-4">
							{history.map((segment, index) => (
								<div key={segment.id} className="bg-black/20 rounded-lg p-4 border border-amber-600/30">
									<div className="flex items-center mb-2">
										<span className="text-amber-400 decorative-text font-semibold">Chapter {index + 1}</span>
										{segment.timestamp && (
									...
									<p className="text-amber-100 antique-text leading-relaxed">{segment.text}</p>
								</div>
							))}
						</div>
					</div>
			...
			{!currentSegment && !showPrompt && !isGenerating && (
				...
				>
					<div className="text-center">
						<h1 className="text-6xl font-bold gold-text decorative-text mb-4">WEAVE</h1>
						<p className="text-amber-200 text-xl antique-text mb-8">Interactive Tales of Choice</p>
						...
						<p className="text-amber-300/80 text-sm mt-4 antique-text">Click the mystical book to begin your journey</p>
					</div>
				</div>
			)}
		</div>
